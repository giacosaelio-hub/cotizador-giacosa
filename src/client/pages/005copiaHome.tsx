...
interface Props {
  ...
  navigateToHomeSection?: (sectionId: string) => void; // Opcional: navegación interna
}
...
export default function Home({ precios, onSelect, navigateToHomeSection }: Props) {
  ...
  // Siempre mostrar hasta 12 cuotas sin interés en el banner como pide el objetivo,
  // y si precios tiene una cantidad mayor, se usa el número real:
  ...
  // MAIN CTA: Pasar al cotizador desde el hero o arriba de categorías
  const handlePrimaryCta = () => {
    productsRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
  };

  // --- Handler para "Ver financiación" asegurando scroll al bloque financiación visible ---
  const handleFinanciacionCta = () => {
    // Si está la prop navigateToHomeSection (ej: Next.js con smooth scroll externo) la usa
    if (typeof navigateToHomeSection === "function") {
      navigateToHomeSection("formas-pago");
    }
    // Si no, scrollea localmente al bloque con id formas-pago
    else {
      const el = document.getElementById("formas-pago");
      if (el) {
  ...
  // Mostrar íconos según método de pago
  function iconoFormaPago(t: { id: string; label: string }) {
    const label = t.label.toLowerCase();
    if (label.includes("efectivo") || label.includes("transferencia")) {
      return "💵";
    }
    if (label.includes("tarjeta") || label.includes("crédito") || label.includes("débito")) {
      return "💳";
    }
    return null;
  }

  // --- getSortedTarjetas está definido pero no se usa, se deja como referencia por si se requiere orden en el futuro ---
  function getSortedTarjetas(tarjetasArr: typeof tarjetas) {
    if (!Array.isArray(tarjetasArr) || tarjetasArr.length === 0) return tarjetasArr;
    const tarjetasDecoradas = tarjetasArr.map(t => ({
      ...t,
      best: Math.min(...t.cuotas.map(c => c.porcentaje)),
    }));
    const quincers = tarjetasDecoradas.filter(t => t.best === -15);
    const others = tarjetasDecoradas.filter(t => t.best !== -15);
    return tarjetasArr;
  }
    ...
    <div className="max-w-3xl mx-auto px-4">
      ...
      <section className="text-center pt-12 pb-8">
        <h1 className="text-3xl sm:text-4xl font-black text-gray-900 leading-tight mb-3">
          Cotizá chapas y bobinas <br />
          <span style={{ color: "#008C45" }}>al instante en Tucumán</span>
        </h1>
        <h2 className="text-base font-semibold text-gray-600 mb-2">
          Giacosa Elio — Corralón y Materiales para la Construcción
        </h2>
        <button
          onClick={handlePrimaryCta}
          className="group transition focus:outline-none inline-flex items-center justify-center px-6 py-3 rounded-2xl bg-green-700 hover:bg-green-800 text-white text-lg font-bold shadow-lg mb-2"
        >
          Cotizar ahora
        </button>
        <div className="flex items-center justify-center mt-2 mb-3">
          <span className="text-sm text-gray-500">Si ya sabés qué necesitás, podés cotizar en segundos.</span>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 text-xs text-gray-400 font-medium">
          ...
          <span className="flex items-center gap-1"><span>💵</span> Hasta 20% OFF en efectivo</span>
          <span className="text-gray-200">|</span>
          <span className="flex items-center gap-1"><span>💳</span> Hasta {cuotasSinRecargo} cuotas sin interés</span>
          <span className="text-gray-200">|</span>
      ...
      >
        ...
        <div className="text-center text-xs text-gray-500 font-semibold mt-3">
          Podés agregar más de un producto antes de confirmar.
        </div>
      </div>

      {/* ── BANNER BENEFICIOS / MEDIOS DE PAGO ───────────────────── */}
      <div
        className="rounded-2xl px-5 py-4 mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"
        style={{ background: "linear-gradient(135deg, #005c2e 0%, #008C45 100%)" }}
      >
        <div>
          <p className="text-white font-bold text-base mb-0.5">
            <span role="img" aria-label="Efectivo" className="mr-1">💵</span>
            Hasta 20% OFF en efectivo
          </p>
          <p className="text-green-200 text-sm">
            <span role="img" aria-label="Tarjeta" className="mr-1">💳</span>
            Hasta {cuotasSinRecargo} cuotas sin interés
          </p>
        </div>
        <button
          onClick={handleFinanciacionCta}
      ...
      {/* ── FINANCIACIÓN ─────────────────────────────── */}
      {/* REGLA IMPORTANTE: El bloque de financiación SIEMPRE debe renderizarse y tener id="formas-pago" */}
      <section
        id="formas-pago"
        className="scroll-mt-20 mb-10 bg-white rounded-2xl border border-gray-200 overflow-hidden"
        style={{ marginTop: "-0.5rem" }}
      >
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          <h2 className="font-bold text-gray-900 text-lg mb-0.5">Formas de pago</h2>
          <p className="text-sm text-gray-400 mb-1">Seleccioná tu tarjeta para ver las cuotas disponibles</p>
          <div className="mt-1 text-green-800 text-sm font-bold flex items-center gap-2">
            <span role="img" aria-label="Efectivo" className="text-lg">💵</span>
            <span>Efectivo</span>
            <span className="bg-green-100 text-green-700 text-xs font-bold rounded-md px-1.5 py-0.5 ml-1">20% OFF</span>
          </div>
        </div>

        {/* Mostrar listado de tarjetas, o si no hay nada, solo efectivo */}
        {tarjetas.length > 0 ? (
          <>
          ...
          <div className="px-5 py-5 text-center text-sm text-gray-400">
            Efectivo y transferencia: 20% OFF.
          </div>
        )}
      </section>
